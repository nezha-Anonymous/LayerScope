...
class FiddlerMoon:
    ...
    def prefetch_experts(self, current_layer):
        ...
        for i in range(min(expert_count, len(layer_hot_experts))):
            ...
            #         if not self.is_expert_in_gpu_now(( self.prefetch_layers +1)%26, next_expert_id):
            #             tick=time.time()
            #             self._async_load_expert(( self.prefetch_layers +1)%26, next_expert_id)
            #             print(f"预取层+1 { self.prefetch_layers } 专家 {next_expert_id} 花费时间 {time.time()-tick}")
            #             self.expert_loc_now[( self.prefetch_layers +1)%26, next_expert_id] = 1
            #             experts_loaded += 1
            #             break
    ...
    def bring_expert_to_gpu(self):
        ...
        try:
            ...
            with open('test.txt', 'a') as f:
                f.write(f"模型: moom, batch_size: {self.batch_size}, 成功加载专家数量: {expert_count}\n")
                
        except RuntimeError as e:
            if 'out of memory' in str(e).lower():
                # 记录显存溢出时的专家数量
                with open('test.txt', 'a') as f:
                    f.write(f"模型: moom, batch_size: {self.batch_size}, 显存溢出时专家数量: {expert_count}\n")
                raise  # 重新抛出异常
            else:
                raise  # 其他异常直接抛出
        ...
        for i_layer, layer in enumerate(self.model.layers):
            ...
            if i_layer == 0:
                ...
                if i_layer < self.n_layer - 1:  # 如果不是最后一层
                    ...
                    # 使用当前层输出预测下一层专家
                    with torch.no_grad():
                        next_predicted_experts,next_routing_weights = next_layer.mlp.gate(inps)  # Moon模型使用mlp.gate
